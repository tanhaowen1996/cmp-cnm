...
class nstestlicense(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(nstestlicense_response, response, self.__class__.__name__)
	...
	def apply(cls, client, resource="") :
		r""" Use this API to apply nstestlicense.
		"""
		try :
			if type(resource) is not list :
				applyresource = nstestlicense()
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the nstestlicense resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = nstestlicense()
...
class nstestlicense_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.nstestlicense = [nstestlicense() for _ in range(length)]
