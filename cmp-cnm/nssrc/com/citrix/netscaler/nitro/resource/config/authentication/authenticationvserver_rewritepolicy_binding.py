...
class authenticationvserver_rewritepolicy_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(authenticationvserver_rewritepolicy_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.authenticationvserver_rewritepolicy_binding
		except Exception as e :
			raise e
	...
	def filter_add_parameters(cls, resource) :
		...
		"""
		addresource = authenticationvserver_rewritepolicy_binding()
		...
		try :
			if resource and type(resource) is not list :
				...
				if resource and len(resource) > 0 :
					updateresources = [authenticationvserver_rewritepolicy_binding() for _ in range(len(resource))]
	...
	def filter_delete_parameters(cls, resource) :
		...
		"""
		deleteresource = authenticationvserver_rewritepolicy_binding()
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				...
				if resource and len(resource) > 0 :
					deleteresources = [authenticationvserver_rewritepolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch authenticationvserver_rewritepolicy_binding resources.
		"""
		try :
			if not name :
				obj = authenticationvserver_rewritepolicy_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of authenticationvserver_rewritepolicy_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count authenticationvserver_rewritepolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = authenticationvserver_rewritepolicy_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of authenticationvserver_rewritepolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = authenticationvserver_rewritepolicy_binding()
...
class authenticationvserver_rewritepolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.authenticationvserver_rewritepolicy_binding = [authenticationvserver_rewritepolicy_binding() for _ in range(length)]
